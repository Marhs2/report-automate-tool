...
| 변수                | 기본값                     | 설명                                                                                                                                                                                                                                                                                 |
| ------------------- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `REPORT_MODEL_NAME` | `qwen3.5-4b`               | LM Studio에 로드한 모델명. **qwen3.5-4b (기본)** — gold 데이터셋 기준 완료 업무 최고 정확도 (91.8%)·이슈 오탐 최소 (문서 `docs/모델-도구-선정-근거.md`)                                                                                                                                                              |
...
| `DAILY_REASONING`   | `none`                     | `none`/`low`/`medium`/`high`. 기본 `none` = 요청에 `reasoning_effort:"none"`을 명시적으로 실어 추론을 끈다(필드 생략은 LM Studio/qwen이 추론을 켜므로 금지). qwen3.5-4b는 gold 데이터셋 기준 완료 업무 최고 정확도 (91.8%). nuextract3로 바꿀 경우에만 `high` 고려 (문서 `docs/모델-도구-선정-근거.md`) |
...
1. **보고서 작성** — 퇴근 전에 그날 업무를 자유 텍스트로 작성/붙여넣기 (양식 없음). 작성자·날짜 지정.
   - 같은 사람이 같은 날 다시 제출하면 덮음.
2. **구조화 확인·수정** — AI가 추출한 프로젝트명/완료/진행/이슈/협조 요청/다음 계획을 화면에서
   확인하고 잘못된 부분을 바로잡아 **저장**한다. 오른쪽에 원문이 그대로 보여 대조.
   - 재추출 버튼으로 AI 추출을 다시 시도할 수 있고, 실패해도 원문은 유실되지 않음.
3. **모아 보기** — 날짜별(프로젝트 목록 화면), 프로젝트별 흐름(타임라인), 활동 기록(사람별·요일별 제출 현황).
   - 표기가 다른 프로젝트명(예: `A사 MES` / `A사 MES 구축`)은 **별칭 관리** 화면에서 한 이름으로 묶음.
4. **주간보고 생성** — 주 마지막 날, 기간(기본 이번 주 월~금, 이전/다음 주 이동 가능)을 선택해 생성.
   - 같은 사람이 같은 기간으로 다시 생성하면 이전 초안을 덮음.
   - 여러 날 반복된 같은 업무는 한 항목으로 병합되고, 미해결 이슈는 별도 표시.
5. **초안 다듬기·내려받기** — 생성된 초안을 편집 화면에서 수정·저장한 뒤,
   **화면 복사** 또는 **워드(.docx) 파일 다운로드**로 제출.
...
**결과 요약 (2026-08-06 재측정, 테스트 보고 32건 × 2데이터셋, 추론 끄기)**

- **gold 데이터셋 32건** (상세: [모델·도구 선정 근거](docs/모델-도구-선정-근거.md)

| 모델 / 설정                       | micro F1  | 보고 1건 평균 |
| --------------------------------- | --------- | ------------- |
| nuextract3 + 추론 끄기            | **82.6%** | **2.54초**    |
| **qwen3.5-4b + 추론 끄기 (채택)** | 81.2%     | 3.04초        |
| google/gemma-4-e2b + 추론 끄기    | 76.5%     | 2.74초        |

- **자체 작성 테스트 보고 데이터셋 32건** : nuextract3 **77.6%** / gemma 76.4% / qwen 73.3%
  - 두 데이터셋 모두 nuextract3가 종합 1위이나, qwen3.5-4b는 완료 업무(91.8%)·이슈 정밀도(81.2%)·
    협조 요청(75.0%)이 세 모델 중 가장 높고 오탐이 적어 기본 모델로 유지한다.
