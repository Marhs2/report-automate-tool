...
샘플 원문과 정답 구조를 실제 프로젝트 목록·사용자 활동 화면에서 확인하려면 다음 명령으로
로컬 SQLite에 적재한다. 같은 사용자·날짜의 기존 샘플 보고서는 애플리케이션의 덮어쓰기
규칙에 맞춰 교체하며, 그 외 데이터는 유지한다.

```bash
python backend/seed_demo_data.py
```

다른 DB에서 먼저 확인할 때는 `--db`로 임시 SQLite 경로를 지정할 수 있다.
